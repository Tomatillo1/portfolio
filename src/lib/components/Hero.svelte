...
>
  ...
  -->
    <div
        class="flex flex-col justify-center px-4 sm:px-6 lg:px-12 xl:px-24 h-full py-20 lg:py-0 overflow-hidden"
        role="region"
        aria-label="Zone de présentation"
    >
        <!-- Conteneur qui garde le texte devant le Spotlight -->
        <div class="flex flex-col md:items-start text-center md:text-left">
            <!-- Nom "Thomas Farkas" injecté via i18n -->
            <h1
                class="font-heading font-semibold text-5xl sm:text-6xl md:text-7xl lg:text-7xl mb-4 {$theme ===
                'dark'
                    ? 'text-text-dark'
                    : 'text-title'}"
            >
          ...
          {$theme === 'dark'
                    ? 'text-text-dark/80'
                    : 'text-text-light/80'} mx-auto md:mx-0"
            >
                {$t.hero.description}
            </p>
          ...
          hover:bg-[#e56030] hover:shadow-xl hover:shadow-accent/30
  ...
  -->
    <div
        class="hidden lg:flex relative items-center justify-center w-full h-full min-h-125"
    >
    ...
    >
        <ChevronDown
            size={24}
            class={$theme === "dark" ? "text-white" : "text-subtitle"}
        />
    </a>
</section>
